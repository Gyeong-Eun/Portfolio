...
- 기존 선정적 헤드라인의 평균 선정성 수치는 0.99였으나, 기사 본문을 반영하여 새로 생성한 헤드라인의 평균 선정성 수치는 0.25로 74.7% 감소
<br>
...
**주요 역할 및 기여**
데이터 수집 전처리 : 뉴스 언론사의 기사를 크롤링하여 제목, 본문과 같은 기사 데이터를 수집하고 학습 데이터 형태에 맞게 전처리
판별 모델 파인튜닝 : LMKor-BERT 기반 선정성 판별 모델의 성능 향상을 위한 파인튜닝 수행
생성 모델 프롬프트 엔지니어링 : GPT-4 Turbo가 비선정적 헤드라인을 생성하지 못하는 문제를 해결하기 위해 선정성 수치와 헤드라인 예시를 활용한 프롬프트 설계
<br>
...
## 7. 기사 제목 선정성 검사기 서비스 시연
- Streamlit을 활용하여 기사 제목 선정성 검사기 서비스를 구현하였다.
- 기사 제목을 입력하고 '검사하기'를 클릭하면 검사 결과로 선정성 여부와 선정성 확률 수치, 선정적인 이유를 제공한다.
- 선정성이 완화된 새로운 제목을 생성하여 제안한다.
<br>

**기존의 선정적 헤드라인 예시**
<p>
 <img src="https://github.com/user-attachments/assets/2865c3f8-e28f-4fe7-a98e-2392ee14085e"width="400"/>
</p>

**선정성이 완화된 새로운 제목 제안**
<p>
 <img src="https://github.com/user-attachments/assets/ab36d777-e2a3-433f-b222-42f7e627e3b7" width="400"/>
</p>
<br>
